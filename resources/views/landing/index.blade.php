...
@section('content')
    ...
    <section id="hero" class="relative min-h-screen flex items-center pt-24 pb-16 overflow-hidden">
        ...
        @endif
        <div class="absolute inset-0 bg-gradient-to-b from-luxury-purple/20 via-transparent to-[#0a0a0f]"></div>
        <div class="absolute top-1/4 -left-20 h-96 w-96 rounded-full bg-luxury-purple/20 blur-[120px]"></div>
        <div class="absolute bottom-1/4 -right-20 h-96 w-96 rounded-full bg-gold/10 blur-[120px]"></div>
        ...
        <div class="relative z-20 mx-auto grid max-w-7xl gap-12 px-4 lg:grid-cols-2 lg:px-8 lg:items-center">
            <div>
                ...
                <div class="hero-jackpot mt-10 glass-gold glow-pulse inline-block rounded-2xl px-8 py-5">
                    <p class="text-sm uppercase tracking-widest text-gold/80">{{ $hero['jackpot_label'] }}</p>
            ...
            <div class="grid grid-cols-2 gap-6 lg:grid-cols-4">
                @foreach([
                    ...
                    <div class="glass-gold rounded-2xl p-6 text-center" data-aos="fade-up">
                        <i class="fas {{ $stat['icon'] }} text-3xl text-gold"></i>
            ...
            <div class="mt-16 grid gap-8 md:grid-cols-2 lg:grid-cols-4">
                @foreach($steps as $step)
                    <div class="group glass-gold rounded-2xl p-6 transition-all duration-300 hover:-translate-y-2 hover:glow-gold" data-aos="fade-up" data-aos-delay="{{ $loop->index * 100 }}">
                        <div class="flex h-14 w-14 items-center justify-center rounded-xl bg-gold/10 text-2xl text-gold transition group-hover:scale-110">
                            <i class="fas {{ $step->icon }}"></i>
                        </div>
                        <span class="mt-4 inline-block rounded-full bg-gold/10 px-3 py-0.5 text-xs font-bold text-gold">Step {{ $step->step_number }}</span>
        ...
        <div class="mx-auto max-w-7xl px-4 lg:px-8">
            ...
            <div class="mt-16 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                @foreach($features as $feature)
                    <div class="glass rounded-2xl p-6 transition hover:border-gold/40 hover:-translate-y-1" data-aos="fade-up" data-aos-delay="{{ $loop->index * 80 }}">
                        <div class="flex h-12 w-12 items-center justify-center rounded-lg bg-gold/10 text-gold text-xl">
                        ...
                        <div class="swiper-slide">
                            <div class="glass-gold rounded-2xl p-6 h-full">
                                <div class="flex items-center gap-4">
                                    ...
                                    @else
                                        <div class="flex h-14 w-14 items-center justify-center rounded-full bg-gold/20 text-gold font-bold text-xl">{{ substr($t->name, 0, 1) }}</div>
                                    @endif
                                    <div>
            ...
            <div class="space-y-4">
                @foreach($faqs as $faq)
                    <div class="faq-item glass rounded-xl overflow-hidden" data-aos="fade-up">
                        <button class="faq-question flex w-full items-center justify-between p-5 text-left font-semibold hover:text-gold transition">
                            {{ $faq->question }}
                            <i class="faq-icon fas fa-plus text-gold transition-transform"></i>
                        </button>
                ...
                <div>
                    ...
                    <div class="mt-8 flex flex-wrap gap-4">
                        <a href="{{ $download['google_play_url'] ?? '#' }}" class="flex items-center gap-3 rounded-xl bg-white/10 px-5 py-3 transition hover:bg-white/20">
                            <i class="fab fa-google-play text-2xl text-gold"></i>
                        ...
                        <a href="{{ $download['app_store_url'] ?? '#' }}" class="flex items-center gap-3 rounded-xl bg-white/10 px-5 py-3 transition hover:bg-white/20">
                            <i class="fab fa-apple text-2xl text-gold"></i>
