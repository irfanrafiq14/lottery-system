<footer id="contact" class="relative border-t border-white/10 bg-[#08080c] py-16">
    <div class="mx-auto max-w-7xl px-4 lg:px-8">
        <div class="grid gap-12 md:grid-cols-4">
            <div class="md:col-span-2">
                ...
                <div class="mt-6 flex gap-4">
                    @foreach($footer['social'] ?? [] as $platform => $url)
                        <a href="{{ $url }}" class="flex h-10 w-10 items-center justify-center rounded-full bg-white/5 text-white/60 transition hover:bg-[#FFD700]/20 hover:text-[#FFD700]" aria-label="{{ $platform }}">
            ...
            <div>
                <h4 class="font-semibold text-[#FFD700]">Quick Links</h4>
            ...
            <div>
                <h4 class="font-semibold text-[#FFD700]">Contact</h4>
                <ul class="mt-4 space-y-2 text-sm text-white/60">
                    <li><i class="fas fa-envelope mr-2 text-[#FFD700]"></i>{{ $footer['contact_email'] ?? '' }}</li>
                    @if(!empty($footer['contact_phone']))
                        <li><i class="fas fa-phone mr-2 text-[#FFD700]"></i>{{ $footer['contact_phone'] }}</li>
                    @endif
                </ul>
            </div>
        </div>
